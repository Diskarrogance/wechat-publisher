...
基于腾讯混元 AI（HY-Image-V3.0）生成封面/配图，自动化完成「搜索→防重→写稿→配图→排版→创建草稿」全流程。每天自动发布：君寻 2 篇 / 岚牧哒 1 篇。
...
| 🎨 **AI 配图** | 腾讯混元 HY-Image-V3.0 生成封面 + 正文配图，IP 角色一致性 |
| 📝 **自动排版** | 预设 HTML 模板（15px 字体、蓝渐变标题、自适应配图尺寸） |
| 🌍 **多源抓取** | 君寻：中文科技资讯；岚牧哒：英文科技媒体翻译 |
| 🔁 **编码安全** | PowerShell CP936 乱码保护，`@file` 临时文件传参 |
| 🖼️ **企业群二维码** | 君寻文章底部自动追加，居中放置 |
...
> ⚠️ 上述两个文件**均已在 `.gitignore` 中排除**，不会误上传
...
python scripts/create_draft.py junxun "@$tmp"
...
```
❗ 禁止：python create_draft.py junxun $json_str
✅ 正确：python create_draft.py junxun "@$tmpFile"  （写 UTF-8 临时文件）
```
